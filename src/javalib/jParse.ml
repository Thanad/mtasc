...
let parse_access_flags ch =
	let all_flags = [ AccPublic; AccPrivate; AccProtected; AccStatic; AccFinal; AccSynchronized; AccVolatile; AccTransient; AccNative; AccInterface; AccAbstract; AccStrict ] in
	...
	if fl land (0x10000 - (1 lsl !fbit)) <> 0 then error ("Invalid access flags " ^ string_of_int fl);
	!flags
